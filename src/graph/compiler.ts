...
export function compileSockets(
  ...
  const solo: SoloCapture | undefined = opts.soloNodeId
    ? {
        id: opts.soloNodeId,
...
function compileDocument(
  ...
  if (solo && solo.value === undefined) {
    ...
    const outputs = node ? nodePorts(node, registry).outputs : [];
    const output = outputs.find((port) => port.key === solo.outputKey) ?? outputs[0];
    if (node && output) {
      solo.value = outputsByNode.get(solo.id)?.[output.key];
      solo.kind = output.kind;
    }
  }
